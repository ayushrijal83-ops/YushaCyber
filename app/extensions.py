...
application inside the app factory (``app/__init__.py``) via
``<extension>.init_app(app)``. This avoids circular imports and keeps a
single instance of each extension that every module can import safely::
...
# Schema migrations (Alembic) — `flask db migrate` / `flask db upgrade`.
migrate = Migrate()
